...
rnd_old = tf.load_op_library('fix_round.so')
...
fPrecision = tf.constant([5.,15.])
bPrecision = tf.constant([5.,14.])
...
with tf.name_scope("ConvPool1"):
	...
	with tf.name_scope("input"):
		x_image = rnd.fix_round_split(tf.reshape(x, [-1,28,28,1]),fPrecision,bPrecision) #28x28 input image with 1 color channel, the -1 just makes sure that the total size is kept constant during reshaping

	h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1) #convolution 1 (includes relu)
	h_pool1 = rnd.fix_round_split(max_pool_2x2(h_conv1),fPrecision,bPrecision) #maxpool 1
...
h_pool2 = rnd.fix_round_split((max_pool_2x2(h_conv2)),fPrecision,bPrecision) #maxpool 2
...
h_fc1 = rnd.fix_round_split(tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1),fPrecision,bPrecision)
...
h_fc2 = rnd.fix_round_split(tf.nn.relu(tf.matmul(h_fc1_flat, W_fc2) + b_fc2),fPrecision,bPrecision)
...
y_conv = rnd.fix_round_split((tf.matmul(h_fc2, W_fc3) + b_fc3),fPrecision,bPrecision)
...
for i in range(1000):
	...
	#sess.run(tf.assign(fPrecision, [5.,15. - (int)(i*0.05)]))
	#print sess.run(fPrecision)
