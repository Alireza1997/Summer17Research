...
with tf.name_scope("ConvPool1"):
	...
	h_conv1 = tf.nn.relu(conv2d(x_image, rnd.fix_round(W_conv1,ILFL)) + rnd.fix_round(b_conv1,ILFL)) #convolution 1 (includes relu)
...
h_conv2 = tf.nn.relu(conv2d(h_pool1, rnd.fix_round(W_conv2,ILFL)) + rnd.fix_round(b_conv2,ILFL)) #convolution 2 (includes relu)
...
h_fc1 = rnd.fix_round(tf.nn.relu(tf.matmul(h_pool2_flat, rnd.fix_round(W_fc1,ILFL)) + rnd.fix_round(b_fc1,ILFL)),ILFL)
...
h_fc2 = rnd.fix_round(tf.nn.relu(tf.matmul(h_fc1_flat, rnd.fix_round(W_fc2,ILFL)) + rnd.fix_round(b_fc2,ILFL)),ILFL)
...
y_conv = rnd.fix_round((tf.matmul(h_fc2, rnd.fix_round(W_fc3,ILFL)) + rnd.fix_round(b_fc3,ILFL)),ILFL)
