...
rnd = tf.load_op_library('fix_round_split.so')
...
def _fix_round_split_grad(op, grad):
  ...
  return rnd.fix_round_split_grad(grad,op.inputs[0],op.inputs[1],op.inputs[2])
...
# ILFLU = tf.constant([5.,15.])
# ILFLF = tf.constant([5.,1.])
# ILFLB = tf.constant([5.,15.])
ILFLU = tf.Variable([1.,7.] ,trainable=False)
ILFLF = tf.Variable([5.,3.] ,trainable=False)
ILFLB = tf.Variable([1.,7.] ,trainable=False)
...
with tf.name_scope("ConvPool1"):
	...
	with tf.name_scope("input"):
		x_image = rnd.fix_round_split(tf.reshape(x, [-1,28,28,1]),ILFLF,ILFLB) #28x28 input image with 1 color channel, the -1 just makes sure that the total size is kept constant during reshaping

	h_conv1 = tf.nn.relu(conv2d(x_image, rnd.fix_round_split(W_conv1,ILFLU,ILFLU)) + rnd.fix_round_split(b_conv1,ILFLU,ILFLU)) #convolution 1 (includes relu)
	h_pool1 = rnd.fix_round_split(max_pool_2x2(h_conv1),ILFLF,ILFLB) #maxpool 1

#convolution and pooling 2
W_conv2 = weight_variable([5,5,20,40],'W2') #5x5 filter, 20 input channels, 40 output channels
b_conv2 = bias_variable([40],'B2')

h_conv2 = tf.nn.relu(conv2d(h_pool1, rnd.fix_round_split(W_conv2,ILFLU,ILFLU)) + rnd.fix_round_split(b_conv2,ILFLU,ILFLU)) #convolution 2 (includes relu)
h_pool2 = rnd.fix_round_split((max_pool_2x2(h_conv2)),ILFLF,ILFLB) #maxpool 2
...
h_fc1 = rnd.fix_round_split(tf.nn.relu(tf.matmul(h_pool2_flat, rnd.fix_round_split(W_fc1,ILFLU,ILFLU)) + rnd.fix_round_split(b_fc1,ILFLU,ILFLU)),ILFLF,ILFLB)
...
h_fc2 = rnd.fix_round_split(tf.nn.relu(tf.matmul(h_fc1_flat, rnd.fix_round_split(W_fc2,ILFLU,ILFLU)) + rnd.fix_round_split(b_fc2,ILFLU,ILFLU)),ILFLF,ILFLB)
...
y_conv = rnd.fix_round_split((tf.matmul(h_fc2, rnd.fix_round_split(W_fc3,ILFLU,ILFLU)) + rnd.fix_round_split(b_fc3,ILFLU,ILFLU)),ILFLF,ILFLB)
...
for i in range(1000):
	...
	if i%chunk == 0:
		...
		avg_accuracy = sum_accuracy/chunk
		sum_accuracy = 0
		print(avg_accuracy)

		if (avg_accuracy < avg_accuracy_old):
