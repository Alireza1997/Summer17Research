...
final_steps = 0						#how many steps at the end of training should be done at full precision
...
for i in range(epochs):
	...
	#continue training and manipulate ILFLs based on accuracy trends
		...
		if (i% accuracy_chunk == 0):
			...
			if (avg_accuracy < avg_accuracy_old*accuracy_tolerance and avg_accuracy < accuracy_peak):
				ilflu = sess.run(ILFLU)
				ilflf = sess.run(ILFLF)
				ilflb = sess.run(ILFLB)
