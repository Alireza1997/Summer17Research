...
ILFLU = tf.Variable([0.,26.] ,trainable=False)
ILFLF = tf.Variable([0.,26.] ,trainable=False)
ILFLB = tf.Variable([0.,26.] ,trainable=False)
...
for i in range(1000):
	...
	if (i % 50) == 0:
		...
		if ((ow[0] > 0) or (ob[0] > 0)):
			...
			sess.run(tf.assign(overflowW, [0,0]))
			sess.run(tf.assign(overflowB, [0,0]))
		if (og[0] > 0):
			print sess.run(tf.assign(ILFLB,tf.add(ILFLB,[1,-1])))
			sess.run(tf.assign(overflowG, [0,0]))
		if (oa[0] > 0):
			print sess.run(tf.assign(ILFLF,tf.add(ILFLF,[1,-1])))
			sess.run(tf.assign(overflowA, [0,0]))
