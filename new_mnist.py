...
W_conv1 = weight_variable([5,5,1,20]) #5x5 filter, 1 input channel, 20 output channels
...
W_conv2 = weight_variable([5,5,20,40]) #5x5 filter, 20 input channels, 40 output channels
...
#fully connected
W_fc1 = weight_variable([4*4*40, 1000])#fully connected, 4x4x40 to 6400
...
W_fc2 = weight_variable([1000, 500])#fully connected, 4x4x40 to 6400
...
W_fc3 = weight_variable([500,10]) #reduce the total outputs to 10 different classes
...
for i in range(2000):
	...
	train_step.run(feed_dict={x:batch[0], y_: batch[1]}) #run the training step using new values for x and y_
